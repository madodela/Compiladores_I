...
namespace FastColoredTextBoxNS
{
    public class SyntaxHighlighter: IDisposable
    {
        ...
        public virtual void HighlightSyntax(Language language, Range range)
        {
            switch (language)
            {
            	case Language.Mycode: MyCodeSyntaxHighlight(range);break;
    ...
    brackets.Attributes["left2"].Value == "" || brackets.Attributes["right2"].Value == "")
		...
		Regex MyCodeStringRegex, MyCodeCommentRegex1, MyCodeCommentRegex2, MyCodeCommentRegex3, MyCodeNumberRegex, MyCodeAttributeRegex, MyCodeClassNameRegex, MyCodeKeywordRegex;

        void InitMyCodeRegex()
        {
            MyCodeStringRegex = new Regex( @"""""|@""""|''|@"".*?""|(?<!@)(?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexCompiledOption);
            MyCodeCommentRegex1 = new Regex(@"//.*$", RegexOptions.Multiline | RegexCompiledOption);
            MyCodeCommentRegex2 = new Regex(@"(/\*.*?\*/)|(/\*.*)", RegexOptions.Singleline | RegexCompiledOption);
            MyCodeCommentRegex3 = new Regex(@"(/\*.*?\*/)|(.*\*/)", RegexOptions.Singleline | RegexOptions.RightToLeft | RegexCompiledOption);
            MyCodeNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\b0x[a-fA-F\d]+\b", RegexCompiledOption);
            MyCodeAttributeRegex = new Regex(@"^\s*(?<range>\[.+?\])\s*$", RegexOptions.Multiline | RegexCompiledOption);
            MyCodeClassNameRegex = new Regex(@"\b(program|class|struct|enum|interface)\s+(?<range>\w+?)\b", RegexCompiledOption);
            MyCodeKeywordRegex = new Regex(@"\b(program|int|float|double|bool|string|char|if|then|else|fi|do|until|while|write|read)\b", RegexCompiledOption);
        }
        ...
        public virtual void MyCodeSyntaxHighlight(Range range)
        {
            ...
            //
            if (MyCodeStringRegex == null)
                InitMyCodeRegex();
            //string highlighting
            range.SetStyle(StringStyle, MyCodeStringRegex);
            //comment highlighting
            range.SetStyle(CommentStyle, MyCodeCommentRegex1);
            range.SetStyle(CommentStyle, MyCodeCommentRegex2);
            range.SetStyle(CommentStyle, MyCodeCommentRegex3);
            //number highlighting
            range.SetStyle(NumberStyle, MyCodeNumberRegex);
            //attribute highlighting
            range.SetStyle(AttributeStyle, MyCodeAttributeRegex);
            //class name highlighting
            range.SetStyle(ClassNameStyle, MyCodeClassNameRegex);
            //keyword highlighting
            range.SetStyle(KeywordStyle, MyCodeKeywordRegex);
...
//            }
    ...
    {
        Custom, CSharp, VB, HTML, SQL, PHP, JS, Mycode
    }
}
